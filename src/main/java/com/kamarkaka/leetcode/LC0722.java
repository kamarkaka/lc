 ...
 *   right.) To be clear, the string "\/*\/" does not yet end the block comment, as the ending would be overlapping the
 *   beginning.
...
public class LC0722 {
    public List<String> removeComments(String[] source) {
        ...
        for (String line : source) {
            int i = 0;
            while (i < line.length()) {
                if (i < line.length() - 1 && line.charAt(i) == '/' && line.charAt(i + 1) == '/' && !inBlockComment) {
                    break;
                } else if (i < line.length() - 1 && line.charAt(i) == '/' && line.charAt(i + 1) == '*' && !inBlockComment) {
                    inBlockComment = true;
                    i += 2;
                    continue;
                } else if (i < line.length() - 1 && line.charAt(i) == '*' && line.charAt(i + 1) == '/' && inBlockComment) {
                ...
                if (!inBlockComment) sb.append(line.charAt(i));
                i++;
            }
