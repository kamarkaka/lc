...
public class LC0364 {
    ...
    private void findMaxDepth(List<NestedInteger> nestedList, int currDepth) {
        if (currDepth > maxDepth) maxDepth = currDepth;
        for (NestedInteger nestedInteger : nestedList) {
            if (!nestedInteger.isInteger()) {
                findMaxDepth(nestedInteger.getList(), currDepth + 1);
            }
        }
    }
    ...
    private int depthSumInverse(List<NestedInteger> nestedList, int weight) {
        ...
        for (NestedInteger nestedInteger : nestedList) {
            if (nestedInteger.isInteger()) {
                ...
                sum += depthSumInverse(nestedInteger.getList(), weight - 1);
            }
        }
