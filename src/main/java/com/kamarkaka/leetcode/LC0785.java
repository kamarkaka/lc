...
public class LC0785 {
    public boolean isBipartite(int[][] graph) {
        ...
        for (int node = 0; node < graph.length; node++) {
            if (visited[node] == 0) {
                if(!dfs(node, -1, visited, graph)) return false;
            }
        }
    ...
    private boolean dfs(int i, int set, int[] visited, int[][] graph) {
        if(visited[i] != 0 && visited[i] != set) return false;
        if(visited[i] != 0) return true;
        visited[i] = set;
        for (int adj: graph[i]) {
            if(!dfs(adj, -set, visited, graph)) return false;
        }
        return true;
    }
}
