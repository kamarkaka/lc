...
public class LC0901 {
    ...
    private final List<Integer> prevIndex;
    ...
    public int next(int price) {
        int currIndex = nums.size();

        if (nums.isEmpty() || price < nums.getLast()) {
            prevIndex.add(currIndex - 1);
        } else {
            int prev = currIndex - 1;
            while (prev > -1) {
                prev = prevIndex.get(prev);
                if (prev == -1 || nums.get(prev) > price) break;
            }
            prevIndex.add(prev);
        ...
        nums.add(price);
        return currIndex - prevIndex.getLast();
    }
}
