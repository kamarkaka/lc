   ...
   private class RandomizedCollection {
      ...
      public boolean insert(int val) {
         if (!this.index.containsKey(val)) {
            this.index.put(val, new LinkedHashSet<>());
         }
         this.index.get(val).add(this.list.size());
         this.list.add(val);
         return this.index.get(val).size() == 1;
      }

      public boolean remove(int val) {
         if (!this.index.containsKey(val) || this.index.get(val).size() == 0) return false;
         int removeIndex = this.index.get(val).iterator().next();
         this.index.get(val).remove(removeIndex);

         int last = this.list.get(this.list.size() - 1);
         this.list.set(removeIndex, last);
         this.index.get(last).add(removeIndex);
         this.index.get(last).remove(this.list.size() - 1);
