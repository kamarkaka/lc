 ...
 *   smallest index, which is 0. The total cost = 0 + 1 = 1. Notice that workers with index 1 and 2 are common in the first and last 3 workers.
 ...
 *   [2,4]. The lowest cost is 2 (index 0). The total cost = 2 + 2 = 4.
...
public class LC2462 {
    public long totalCost(int[] costs, int k, int candidates) {
        long cost = 0;
        int n = costs.length;
        Set<Integer> hired = new HashSet<>();

        PriorityQueue<int[]> left = new PriorityQueue<>((n1, n2)->{
            if (n1[0] == n2[0]) return n1[1] - n2[1];
            return n1[0] - n2[0];
        });
        PriorityQueue<int[]> right = new PriorityQueue<>((n1, n2)->{
            if (n1[0] == n2[0]) return n1[1] - n2[1];
            return n1[0] - n2[0];
        });
        ...
        for (int i = 0; i < candidates; i++) {
            ...
            right.add(new int[] {costs[n-1-i], n-i-1});
        }

        int pl = candidates;
        int pr = n - candidates - 1;
        ...
        for (int i = 0; i < k; i++) {
            ...
            if (r == null || l[0] <= r[0]) {
                ...
                while (pl < n) {
                    if (!hired.contains(pl)) {
                        left.add(new int[] {costs[pl], pl++});
                        break;
                    }
                    pl++;
                }
            ...
            } else if (l == null || l[0] > r[0]) {
                cost += r[0];
                hired.add(r[1]);
                right.poll();

                while (pr >= 0) {
                    if (!hired.contains(pr)) {
                        right.add(new int[] {costs[pr], pr--});
                        break;
                    }
                    pr--;
                }
