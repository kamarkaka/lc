...
public class LC0721 {
   public List<List<String>> accountsMerge(List<List<String>> accounts) {
      ...
      for (int userId = 0; userId < size; userId++) {
         for (int i = 1; i < accounts.get(userId).size(); i++) {
            String email = accounts.get(userId).get(i);
            if (emailMap.containsKey(email)) {
               dsu.union(userId, emailMap.get(email));
            } else {
      ...
      for (String email : emailMap.keySet()) {
         int userId = emailMap.get(email);
         int parentUserId = dsu.findParent(userId);

         components.putIfAbsent(parentUserId, new ArrayList<>());
         components.get(parentUserId).add(email);
      }

      List<List<String>> result = new ArrayList<>();
      for (int userId : components.keySet()) {
         List<String> emails = components.get(userId);
         Collections.sort(emails);
         emails.addFirst(accounts.get(userId).getFirst());
         result.add(emails);
      }
      return result;
   }
