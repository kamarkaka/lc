...
public class LC1570 {
   class SparseVector {
      ...
      public int dotProduct(SparseVector that) {
         ...
         while (p < this.pairs.size() && q < that.pairs.size()) {
            if (this.pairs.get(p)[0] == that.pairs.get(q)[0]) {
               result += this.pairs.get(p)[1] * that.pairs.get(q)[1];
               p++;
               q++;
            } else if (this.pairs.get(p)[0] > that.pairs.get(q)[0]) {
               q++;
            } else {
               p++;
            }
