 ...
 * of i are larger than nums[i]. Adjacent indices i and j are part of the same hill or valley if nums[i] == nums[j].
    ...
    public int countHillValley(int[] nums) {
        ...
        while (p1 < nums.length - 1) {
            ...
            if (nums[p1 - 1] < nums[p1]) {
                ...
                if (p2 < nums.length && nums[p1] > nums[p2]) count++;
                p1 = p2;
            // valley?
            } else if (nums[p1 - 1] > nums[p1]) {
                p2 = p1 + 1;
                while (p2 < nums.length && nums[p2] == nums[p1]) {
                    p2++;
                }
