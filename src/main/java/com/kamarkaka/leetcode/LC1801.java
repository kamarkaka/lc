...
public class LC1801 {
   public int getNumberOfBacklogOrders(int[][] orders) {
      ...
      PriorityQueue<int[]> buyBacklog = new PriorityQueue<>(Comparator.comparingInt(a -> -a[0]));

      for (int[] order : orders) {
         if (order[2] == 0) {
            // simulate buy case
            while (!sellBacklog.isEmpty() && sellBacklog.peek()[0] <= order[0] && order[1] > 0) {
               int[] sellOrder = sellBacklog.peek();

               // remove the min of these
               int toRemove = Math.min(order[1], sellOrder[1]);
               sellOrder[1] -= toRemove;
               order[1] -= toRemove;
               backLogOrdersAmount -= toRemove;
            ...
            // simulate sell case
            while(!buyBacklog.isEmpty() && buyBacklog.peek()[0] >= order[0] && order[1] > 0) {
               int[] buyOrder = buyBacklog.peek();

               // remove the min of these
               int toRemove = Math.min(order[1], buyOrder[1]);
               buyOrder[1] -= toRemove;
               order[1] -= toRemove;
               backLogOrdersAmount -= toRemove;
