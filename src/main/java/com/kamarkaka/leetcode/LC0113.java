...
public class LC0113 {
    public List<List<Integer>> pathSum(TreeNode root, int sum) {
        ...
        dfs(result, new ArrayList<>(), root, 0, sum);
        return result;
    }

    private void dfs(List<List<Integer>> result, List<Integer> pResult, TreeNode node, int currSum, int sum) {
        if (node == null) return;
        if (isLeaf(node)) {
            if (currSum + node.val == sum) {
                pResult.add(node.val);
                result.add(new ArrayList<>(pResult));
        ...
        pResult.add(node.val);
        dfs(result, pResult, node.left, currSum + node.val, sum);
        dfs(result, pResult, node.right, currSum + node.val, sum);
        pResult.remove(pResult.size() - 1);
    }
