...
public class LC2300 {
    public int[] successfulPairs(int[] spells, int[] potions, long success) {
        ...
        for (int i = 0; i < spells.length; i++) {
            res[i] = find((long) spells[i], potions, 0, potions.length - 1, success);
        }

        return res;
    }

    private int find(long spell, int[] potions, int l, int r, long success) {
        if (l > r) return 0;
        if (l == r) {
            return spell * potions[l] >= success ? 1 : 0;
        }

        int m = l + (r - l) / 2;
        if (spell * potions[m] >= success) {
            return find(spell, potions, l, m - 1, success) + r - m + 1;
        } else {
            return find(spell, potions, m + 1, r, success);
        }
    }
}
